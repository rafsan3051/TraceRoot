...
const USE_REAL_BLOCKCHAIN = process.env.USE_REAL_BLOCKCHAIN !== 'false' // Default to true, only false if explicitly set
...
/**
 * Mock blockchain for development/testing - generates realistic-looking Fabric tx IDs
 */
function mockBlockchainRecord(data) {
  ...
  // Generate a 64-char hex string to mimic Fabric transaction hash
...
export async function recordToBlockchain(data) {
  ...
  try {
    ...
    // Register product on Fabric with a transaction object to capture txId
    const tx = contract.createTransaction('RegisterProduct')
