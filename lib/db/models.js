...
export const User = {
  ...
  async update({ where, data }) {
    ...
    return formatDocument(result?.value)
  },
...
export const SupplyChainEvent = {
  ...
  async create({ data, include }) {
    ...
    const result = await collection.insertOne(doc)
      const formatted = formatDocument({ _id: result.insertedId, ...doc })
    
      // Handle includes if provided
    if (include) {
      if (include.product) {
          formatted.product = await Product.findUnique({ where: { id: formatted.productId } })
        }
      }
...
export const PasswordResetToken = {
  async findUnique({ where }) {
    const collection = await getCollection('PasswordResetToken')
    const query = {}
