 ...
 * @param {string | Request} tokenOrRequest - JWT token string or Next.js request object
 * @returns {Promise<{authenticated: boolean, user: object} | {authenticated: boolean, error: string}>}
 */
export async function verifyAuth(tokenOrRequest) {
  let token = null
  ...
  if (typeof tokenOrRequest === 'string') {
    ...
    // It's a request object - extract token
    token = getAuthToken(tokenOrRequest)
  }
