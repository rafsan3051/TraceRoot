...
export async function recordToBlockchain(data) {
  ...
  try {
    // Direct REST call to Kaleido FabConnect gateway
    ...
    console.log('🔗 Kaleido Config:', { baseUrl, channel, chaincode, hasAuth: !!authHeader })

    if (!baseUrl || !channel || !chaincode || !authHeader) {
      console.error('❌ Missing Kaleido config!')
      return mockBlockchainRecord(data)
    }
    ...
    console.log('📤 RegisterProduct REST call:', { url, func: 'RegisterProduct', args: payload.args.length })
    ...
    console.log('📥 RegisterProduct response:', res.status, body)

    if (!res.ok) {
      console.error('❌ RegisterProduct failed:', res.status, text)
      return mockBlockchainRecord(data)
    }
    ...
    console.error('❌ RegisterProduct error:', error.message, error.stack)
    return mockBlockchainRecord(data)
  }
}
