...
export async function recordToBlockchain(data) {
  ...
  try {
    // Dynamic import of REST client (only in Node.js environments)
    const { invoke } = await import('./fabric-rest.js')
    ...
    const txId = result.txid || result.transactionId || result.id || String(Date.now())
    ...
    console.log('⚠️ Using mock blockchain record')
    return mockBlockchainRecord(data)
  }
}
