...
export async function recordToBlockchain(data) {
  ...
  try {
    // Use Kaleido REST API instead of SDK (works in Vercel)
    const restEndpoint = process.env.KALEIDO_REST_API
    if (!restEndpoint) {
      console.warn('⚠️ KALEIDO_REST_API not set, using mock')
      return mockBlockchainRecord(data)
    }

    const response = await fetch(`${restEndpoint}/transactions`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('❌ REST API error:', response.status, error)
      // Fallback to mock if REST fails
      return mockBlockchainRecord(data)
    }

    const result = await response.json()
    const txId = result.transactionId || result.id || String(Date.now())
    console.log('✅ Product registered on Kaleido:', txId)
    
    return txId
  } catch (error) {
    console.error('❌ Failed to record to blockchain:', error.message)
...
export async function recordEventToBlockchain(productId, eventData) {
  ...
  try {
    ...
    console.error('❌ Failed to record event to blockchain:', error.message)
    // Graceful fallback to mock
    return mockBlockchainRecord({ productId, ...eventData })
  }
}
