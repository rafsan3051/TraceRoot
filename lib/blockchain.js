  ...
  try {
    ...
    try {
      ...
      if (res.ok || res.status === 202) {
        ...
        console.warn('[WARNING] Kaleido status:', res.status, responseBody)
        console.log('[OK] Transaction submitted with ID:', txId)
        return txId
      }
    } catch (fetchError) {
      clearTimeout(timeoutId)
      ...
      console.warn('')
      console.warn('[WARNING] BLOCKCHAIN OFFLINE - GRACEFUL FALLBACK ACTIVE')
      console.log('[OK] Transaction recorded locally with ID:', txId)
